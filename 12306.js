...
(function() {
    ...
    return;
    setTimeout(()=>{
        //console.log( jQuery('#query_ticket'));
        //var obj=document.querySelector('#query_ticket');
        //obj.click();
    },2300);

    //查询
    var settings = {
        "url": "https://kyfw.12306.cn/otn/leftTicket/queryA?leftTicketDTO.train_date=2022-01-21&leftTicketDTO.from_station=GZQ&leftTicketDTO.to_station=PEQ&purpose_codes=ADULT",
        "method": "GET"
    };
    ...
    $.ajax(settings).done(function (response) {
        ...
        var str_arr=arr.pop().split('|');
        var sec=str_arr.shift();
        var  train_location=str_arr[14];
        //申请订单
        settings = {
            "url": "https://kyfw.12306.cn/otn/leftTicket/submitOrderRequest",
            "method": "POST",
            "data": "secretStr="+sec+"&train_date=2022-01-21&back_train_date=2022-01-21&tour_flag=dc&purpose_codes=ADULT&query_from_station_name=广州&query_to_station_name=普宁&undefined",
        };
        ...
        $.ajax(settings).done(function (response) {
            ...
            $.ajax(settings).done(function (response) {
                ...
                var leftTicketStr=/'leftTicketStr':'(.*?)'/.exec(response);
                var key_check_isChange=/'key_check_isChange':'(.*?)'/.exec(response);
                console.log(globalRepeatSubmitToken,leftTicketStr,key_check_isChange,train_location);
                //校验订单
                settings = {
                    "url": "https://kyfw.12306.cn/otn/confirmPassenger/checkOrderInfo",
                    "method": "POST",
                    "data": {
                        "REPEAT_SUBMIT_TOKEN": globalRepeatSubmitToken[1],
                        "_json_att": "",
                        "bed_level_order_num": "000000000000000000000000000000",
                        "cancel_flag": "2",
                        "oldPassengerStr": "张斌,1,4414***********192,1_",
                        "passengerTicketStr": "O,0,1,张斌,1,4414***********192,136****3232,N,523d2997efbe8ba4b0df6f399a79b76b5ba1979744de9e77293b32dc9cd155a8c2361e14878d95de1f5f199d1ea39e6f595e6b58c0dd81df981a84369ebbdf5e79e182e0376630a7b89b60ce6f4dd56d",
                        "randCode": "",
                        "scene": "nc_login",
                        "sessionId": "",
                        "sig": "",
